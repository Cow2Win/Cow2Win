...
public class TitanValueOverviewDialog extends JDialog {
    ...
    private static final String BASE_TITLE = "Cow2 - Titan Teams";
    ...
    /** Target size of the member icons (Titans column). */
    ...
    enum ValueMode {
        ...
        /** Number of titans with the fortification's required element. */
    ...
    private final List<ValueColumn> titanValueColumns = buildValueColumns(titanFortifications);

    /** Currently selected value mode (see {@link #buildValueModeCombo()}). */
    private ValueMode valueMode = ValueMode.COW_SCORE;

    private final TitanValueTableModel titanModel =
            new TitanValueTableModel(COLUMN_KEY_TITANS, titanValueColumns, this::handleFortificationSelected,
    ...
    public TitanValueOverviewDialog(Frame owner, AppContext appContext,
        ...
        configureTable(titanTable, t -> IconLoader.iconFor(t.imagePath(), MEMBER_ICON_SIZE),
        ...
        content.add(new JScrollPane(titanTable), BorderLayout.CENTER);
    ...
    private static void configureTable(JTable table, Function<Titan, Icon> iconResolver, Function<Titan, String> nameResolver,
        ...
        for (int col = TitanValueTableModel.FIXED_COLUMN_COUNT; col < table.getColumnModel().getColumnCount(); col++) {
        ...
        // always exposed as Double (see TitanValueTableModel#getColumnClass),
        ...
        if (table.getRowSorter() instanceof TableRowSorter<?> rowSorter) {
            ...
            sorter.setComparator(2, Comparator.<List<Titan>, String>comparing(
     ...
     * {@link TitanValueTableModel.Row#matchCounts}/{@link TitanValueTableModel.Row#scores}
     ...
     * counterpart of {@code TeamsOverviewPanel#saveGuild()}, scoped to titan
    ...
    /**
     * {@code appContext.guild()} with every titan team's totalPower replaced
     * by what {@link #titanModel} currently shows (see {@code TeamsOverviewPanel#guildWithCurrentSelection()})
     * - hero teams are copied over unchanged, since this dialog has no data
     * for them at all (see {@link HeroValueOverviewDialog} for the hero
     * counterpart of this save).
     */
    private Guild guildWithCurrentSelection() {
        Guild currentGuild = appContext.guild();
        List<TitanValueTableModel.Row> titanRows = titanModel.rows();
        int titanIndex = 0;
        List<GuildMember> updatedMembers = new ArrayList<>();
        for (GuildMember member : currentGuild.members()) {
            List<TitanTeam> updatedTitanTeams = new ArrayList<>();
    ...
    /**
     * {@code ROLE_MATCH_COUNT} data: the number of titans in a synthetic
     ...
     * Javadoc).
     */
    private static double[] scoresFor(TitanTeam team, List<ValueColumn> valueColumns) {
     ...
     */
    static final class TitanValueTableModel extends AbstractTableModel {

        /** Number of fixed columns before the per-fortification value columns start (Power, Member, Titans, Fortification). */
        static final int FIXED_COLUMN_COUNT = 4;

        private final String membersColumnKey;

        /** This table's own value columns, in display order - see {@link TitanValueOverviewDialog#scoresFor}/{@link TitanValueOverviewDialog#matchCountsFor}. */
         ...
         * {@link TitanValueOverviewDialog#handleFortificationSelected}.
         ...
         * was committed for a row - see {@link TitanValueOverviewDialog#handlePowerEdited}.
        ...
        /** Read on every {@link #getValueAt} call for a value column - see {@link TitanValueOverviewDialog#valueMode}. */
        private final Supplier<ValueMode> modeSupplier;

        private List<Row> rows = new ArrayList<>();

        TitanValueTableModel(String membersColumnKey, List<ValueColumn> valueColumns,
         ...
         * - so the row sorter's comparator (see {@link TitanValueOverviewDialog#configureTable})
        ...
        /**
         * One table row = one titan team. totalPower starts at the team's
         ...
         * column edit (see {@link TitanValueOverviewDialog#handlePowerEdited})
         * updates it (and, together with it, matchCounts/scores - both
         * derived from totalPower, see {@link TitanValueOverviewDialog#matchCountsFor}/
         * {@link TitanValueOverviewDialog#scoresFor}), and it is written back
         * into the guild's team the same way on Save (see
         * {@link TitanValueOverviewDialog#guildWithCurrentSelection()}).
         */
    ...
    static final class MembersCellRenderer implements TableCellRenderer {
        ...
        private final Function<Titan, Icon> iconResolver;
        private final Function<Titan, String> nameResolver;

        MembersCellRenderer(Function<Titan, Icon> iconResolver, Function<Titan, String> nameResolver) {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected,
            ...
            @SuppressWarnings("unchecked")
            List<Titan> members = (List<Titan>) value;
            if (members != null) {
                for (Titan member : members) {
     ...
     * {@link ValueMode#ROLE_MATCH_COUNT} (see {@link TitanValueTableModel#mode()}) -
    ...
    static final class ValueCellRenderer implements TableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected,
            ...
            TitanValueTableModel model = (TitanValueTableModel) table.getModel();
